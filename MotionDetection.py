...
class MotionDetectionApp(QWidget):
    ...
    def set_roi(self):
        ...
        if not ret:
            print("Error: Failed to capture a frame for ROI selection.")
            return  # If frame capture fails, stop the function.
    ...
    def start_motion_detection(self):
        while True:
            ...
            if self.normal_mode:
                ...
                threshold_value = sensitivity * 10  # Adjust threshold dynamically
                threshold = cv2.threshold(difference, threshold_value, 255, cv2.THRESH_BINARY)[1]

                self.start_frame = frame_roi

                cv2.imshow("Cam", threshold)

                if threshold.sum() > threshold_value:
                    print("Motion Detected")
                    self.motion_logger.add_event()
                    send_mac_notification("Motion Detected", "Motion has been detected!")
